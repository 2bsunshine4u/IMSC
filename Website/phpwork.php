...
EOF;*/
    ...
    while($row = pg_fetch_row($ret)){
      ...
      $gps_pattern = explode(",", $ex_row3);//pattern3
      $ex_row4 = substr($row[8], 1, strlen($row[8])-2);
      $si1 = explode(",", $ex_row4);//similarity1
      $ex_row5 = substr($row[9], 1, strlen($row[9])-2);
      $si2 = explode(",", $ex_row5);//similarity2
      ...
      foreach ($gps_pattern as $key => $value) {
        if($value=="NULL")
          $gps_pattern[$key] = null;
      }
    }
  pg_close($db);  
  }
...
<table border='1' align='center' >
  ...
  <tr>
    <?php
      echo "<td>Realtime&Production</td><td>".$si1[1]."</td><td>".$si1[2]."</td><td>".$si1[3]."</td><td>".$si1[4]."</td><td>".$si1[0]."</td>";
    ?>
  </tr>
  <tr>
    <?php
      echo "<td>Realtime&V1</td><td>".$si2[1]."</td><td>".$si2[2]."</td><td>".$si2[3]."</td><td>".$si2[4]."</td><td>".$si2[0]."</td>";
    ?>
  </tr>
</table>
</br>
<table align='center'>
  <tr>
    ...
    <th>&nbsp;V1_pattern: </th>
    <th><hr width=50 noshade color="#00EE00"></th>
  </tr>
</table>
...
$(function(){  //js中方法
  ...
  var  gps_pattern = <?php echo json_encode($gps_pattern) ?>;
  ...
  if(realtime_pattern || historical_pattern || gps_pattern){
