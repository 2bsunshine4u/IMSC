...
class Pattern(object):
    ...
    def road_sensor_data(self, road_name, mapping):
        ...
        sensor_loc = {}
        sql = "select distinct sensor_id, ST_X(start_lat_long), ST_Y(start_lat_long) from highway_congestion_config where last_seen_at >= '2015-01-01' and last_seen_at < '2016-01-01' and sensor_id in " + ss
        self.pg_cursor.execute(sql)
        results = self.pg_cursor.fetchall()
        for sensor_id, longitude, latitude in results:
            sensor_loc[sensor_id] = (longitude, latitude)
