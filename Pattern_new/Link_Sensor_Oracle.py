...
class Sensor(object):
    ...
    #find all sensors on hwys
        ...
        if len(set_dir) > 1:
            sql = "select distinct sensor_id, ST_X(start_lat_long), ST_Y(start_lat_long), onstreet from highway_congestion_config where last_seen_at >= '2015-01-01' and last_seen_at < '2016-01-01' and onstreet like '%" + road_name + "%' and direction in " + str(set_dir)
        else:
            sql = "select distinct sensor_id, ST_X(start_lat_long), ST_Y(start_lat_long), onstreet from highway_congestion_config where last_seen_at >= '2015-01-01' and last_seen_at < '2016-01-01' and onstreet like '%" + road_name + "%' and direction = '" + set_dir[0] + "'"
        self.pg_cursor.execute(sql)
        results = self.pg_cursor.fetchall()
        sensors = []
        for sensor_id, longitude, latitude, onstreet in results:
            ...
            t = [sensor_id, (longitude, latitude)]
            ...
            for link in path[section]:
                ...
                for sensor in sensors:
                    ...
                    if (Utils.is_in_bbox(lon1,lat1,lon2,lat2,lon_sen,lat_sen) and Utils.point2line(lon_sen,lat_sen,lon1,lat1,lon2,lat2) < 200):
                        ...
                        dict_road[section][link].append((sensor[0], "t"))
                ...
                if len(dict_road[section][link]) == 0:
                    if dist1 < 450:
                        dict_road[section][link].append((sen1[0], "f"))
                        if sen1 not in used_s:
                            used_s.append(sen1)

                    if dist2 < 450:
                        dict_road[section][link].append((sen2[0], "f"))
...
if __name__ == '__main__':
    ...
    sql = "truncate table sensor_mapping_highway"
        ...
        for section in mapping:
            ...
            for link in mapping[section]:
                ...
                if len(mapping[section][link]) == 0:
                    sql = "insert into sensor_mapping_highway (road_name,direction,from_postmile,to_postmile,link_id,start_nodeid, start_loc,end_nodeid,end_loc,length, way_id) values (:1,:2,:3,:4,:5,:6,SDO_GEOMETRY(2001,NULL,SDO_POINT_TYPE(:7, :8, NULL),NULL,NULL),:9,SDO_GEOMETRY(2001,NULL,SDO_POINT_TYPE(:10, :11, NULL),NULL,NULL),:12,:13)"
                    ...
                    for sensor, on_edge_flag in mapping[section][link]:
                        ...
                        sql = "insert into sensor_mapping_highway (road_name,direction,from_postmile,to_postmile,link_id,start_nodeid,start_loc,end_nodeid,end_loc,length,way_id,sensor_id, sensor_loc, on_edge_flag) values (:1,:2,:3,:4,:5,:6,SDO_GEOMETRY(2001,NULL,SDO_POINT_TYPE(:7, :8, NULL),NULL,NULL),:9,SDO_GEOMETRY(2001,NULL,SDO_POINT_TYPE(:10, :11, NULL),NULL,NULL),:12,:13,:14,SDO_GEOMETRY(2001,NULL,SDO_POINT_TYPE(:15, :16, NULL),NULL,NULL),:17)"
                        data = (road_name,show_dir,from_postmile,to_postmile,link,start_nodeid,start_loc[0],start_loc[1],end_nodeid,end_loc[0],end_loc[1],length,wayid,sensor,senloc[0],senloc[1],on_edge_flag)
                        lamap.insert_oracle(sql,data)
